...
def parseArgs(args=None):
    ...
    parser.add_option('--geode', action='store_false', dest='atom',
                      help='run the geode binaries', default=True)
    parser.add_option('--atom', action='store_true', dest='atom',
                      help='run the atom binaries', default=True)
    parser.add_option('--gdb', action='store_true', dest='gdb',
                      help='run gdb on the binaries (you may also want to use --debug to use the debug binaries', default=False)
    parser.add_option('--valgrind', action='store_true', dest='valgrind',
                      help='run valgrind on the binaries (you may also want to use --debug to use the debug binaries', default=False)
    parser.add_option('--debug', action='store_true', dest='debug',
                      help='use the debug binaries', default=False)
    if args is None:
        args = sys.argv[1:]
        otherArgs = []
        i = 0
