...
public:
  ...
  YaVector(const YaVector<V>& other) : std::vector<V>(other)
  {
    (std::vector<V>&) *this = (std::vector<V>&) other;
    N = other.N;
  }

  YaVector(V x, V y, V z) : std::vector<V>(3), N(3)
  {
    (*this)[0] = x;
    (*this)[1] = y;
    (*this)[2] = z;
  ...
  const YaVector<V> operator+(const YaVector<V>& other) const
  {
  ...
  const YaVector<V> operator-() const {
    YaVector<V> result(N, V());
    for(int i = 0; i < N; i++)
      result[i] = (*this)[i] * -1;
    return result;
...
public:
  ...
  YaMatrix(V a11, V a12, V a13, V a21, V a22, V a23, V a31, V a32, V a33) : M(3), N(3), v(3, YaVector<V>(3, V()))
  {
    v[0][0] = a11; v[0][1] = a12; v[0][2] = a13;
    v[1][0] = a21; v[1][1] = a22; v[1][2] = a23;
    v[2][0] = a31; v[2][1] = a32; v[2][2] = a33;
  }
...
template<class V>
YaVector<V> operator*(const YaMatrix<V>& a, const YaVector<V>& b)
{
  YaVector<V> c(a.N, 0);
  for(int m = 0; m < a.M; m++)
    for(int n = 0; n < a.N; n++)
...
template<class V>
std::ostream& operator<<(std::ostream& stream, const YaVector<V> vec)
{
  for(int n = 0; n < vec.size(); n++) {
    stream << vec[n];
    if(n != vec.size()) stream << " ";
  }
  return stream;
}
...
YaMatrix<T> yaMatrix(int M, int N, T first, ...)
{
  ...
  for(int i = 0; i < M; i++)
    for(int j = 0; j < N; j++)
      if(i != 0 || j != 0)
        matrix[i][j] = (T) va_arg(vl, double);
  va_end(vl);
  return matrix;
}

template<class T>
YaVector<T> yaVector(int N, double first, ...)
{
  YaVector<T> vector(N, 0);
  vector[0] = first;
  va_list vl;
  va_start(vl, first);
  for(int i = 1; i < N; i++)
    vector[i] = (T) va_arg(vl, double);
  va_end(vl);
  return vector;
}
